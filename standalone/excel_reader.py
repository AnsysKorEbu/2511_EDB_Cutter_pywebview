...
from .config import get_height_column
...
def read_material_properties(filename):
    ...
    if not check_file_exists(filename):
        logger.error(f"Cannot read material properties: file not found")
        sys.exit(1)
...
def read_layer_material(filename):
    ...
    """
    if not check_file_exists(filename):
        logger.error(f"Cannot read layer material: file not found")
        sys.exit(1)
    ...
    while True:
        # Check if current row has data (column 5)
        check_value = ws.cell(row=current_row, column=5).value
        if check_value is None:
            break
        ...
        material_name = find_real_value(ws, current_row, 5)  # material (column 5)
...
if __name__ == "__main__":
    print("Standalone Stackup Data Extractor - Test Mode")
    print("=" * 60)
    ...
    if len(sys.argv) > 1:
        ...
        # Try to find rawdata.xlsx in current directory or parent
        current_dir = Path(__file__).parent
        possible_paths = [
            current_dir / "rawdata.xlsx",
            current_dir.parent / "rawdata.xlsx",
            current_dir.parent / "stackup" / "rawdata.xlsx"
        ]
    ...
    try:
        # Test read_material_properties
        print("1. Testing read_material_properties()...")
        layer_data = read_material_properties(excel_file)
        print(f"   Found {len(layer_data)} material entries")
        if layer_data:
            print(f"   First entry: {layer_data[0]}")
        print()

        # Test read_layer_material
        print("2. Testing read_layer_material()...")
        layer_material_info = read_layer_material(excel_file)
        print(f"   Found {len(layer_material_info)} layer material entries")
        if layer_material_info:
            print(f"   First entry: {layer_material_info[0]}")
        print()
